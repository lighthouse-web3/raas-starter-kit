...
const dealStatusABINew = require("./dealStatusABINew")
...
const submit = async () => {
    ...
    const dealStatusContract = new ethers.Contract(
        "0x5D3aCF96d7234D9290DF048AFD194c4c7DbDdE6F",
        dealStatusABINew,
        signer
    )
    ...
    // let submitTx = await dealStatusContract.submit(ethers.utils.toUtf8Bytes(cid))
    // submitTx = await submitTx.wait()
    // console.log("Submitted")

    // console.log("Complete Function")
    // let completeTx = await dealStatusContract.complete(2, 23232, 3333, incProof, verifData)
    // completeTx = await completeTx.wait()
    // console.log(JSON.stringify(completeTx))
    // console.log( "Complete Function Executed" )

    // console.log("Setting Deals...")
    // let setDealTx = await dealStatusContract.setDeals(ethers.utils.toUtf8Bytes(cid), 136287, 17840)
    // setDealTx = await setDealTx.wait()
    // console.log("Deal Set")

    // let otherDeals = [
    //     136287, 17840, 136291, 17840, 136291, 17840, 136293, 17840, 136297, 17840, 136297, 17840,
    //     136301, 17840, 136300, 17840, 136294, 17840, 136308, 17840, 136310, 17840, 136315, 17840,
    //     136314, 17840, 136318, 17840, 136320, 17840, 136336, 17840, 136344, 17840, 136338, 17840,
    //     136329, 17840, 136346, 17840, 136332, 17840, 136341, 17840, 136326, 17840,
    // ]
    // console.log("Setting few deals..")
    // for (let i = 0; i < 5; i++) {
    //     const dealId = otherDeals[2 * i]
    //     const minerId = otherDeals[2 * i + 1]
    //     console.log("Setting Deal " + (i + 1))
    //     console.log({ dealId, minerId })
    //     let setDealTx = await dealStatusContract.setDeals(
    ...
    console.log("Get active deals")
    let actDeals = await dealStatusContract.getActiveDeals(ethers.utils.toUtf8Bytes(cid))
    console.log("Active Deals : " + actDeals)

    console.log("Get expiring deals")
    let expDeals = await dealStatusContract.getExpiringDeals(ethers.utils.toUtf8Bytes(cid), 1430314)
    console.log("Expiring Deals : " + expDeals)
}
